...
FCS: "least complex" seems to mean the portal with the least number of nummightsee
     The search just loops on all the portals and find the minimun nummightsee.
=============
*/
portal_t *GetNextPortal (void)
{
    ...
    // Iterates numportals*2 times because we created two portals (one in each direction) for each portal loaded
    // fron the prt file.
...
*/
#if defined(__alpha) || defined(__APPLE__)
void *LeafThread (void* threadParameters)
#else
...
/*
 ...
 FCS: Each portal found in the ptr file is bi-directional so for each porta found: Two are created.
